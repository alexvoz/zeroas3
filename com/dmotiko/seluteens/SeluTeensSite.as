package com.dmotiko.seluteens {
	...
	extends WebSite {
		...
		private var seluLoader:SeluTeensLoader;
		...
		public static function getApp():SeluTeensSite {
			return SeluTeensSite(app);
		}
						
		public function SeluTeensSite() {
			super();
		...
		override protected function initSite():void {
			...
			this.sSection = SeluTeensSite.HOME;
			super.initSite();
		...
		private function snd_complete(e:Event):void {
			SeluTeensSite.getApp().log("snd complete");
		}
		...
		override public function setSection( s:String ):void {
			if ( /*s == SeluTeensSite.NOVEDADES || s == SeluTeensSite.PRENSA || */ s== SeluTeensSite.SEXIES ) return;
			super.setSection( s );
		}
		
		override protected function externalContentLoaded( evnt:Event = undefined):void {
			getChildByName("mcSeluTeensLoader").addEventListener( Event.CHANGE, hideLoader);
		...
		private function hideLoader(e:Event):void {
			//getChildByName("mcSeluTeensLoader").visible = false;
			removeChild( getChildByName("mcSeluTeensLoader") );
		}
		
		private function onProgressHandler(evnt:ProgressEvent) {         
			var nP:Number = Math.round( evnt.bytesLoaded * 100 / evnt.bytesTotal );
			(getChildByName("mcSeluTeensLoader") as SeluTeensLoader).setPos( nP );
		}
		...
		public function fadeInMusic():void {
			if ( !getSound() || getSection() == SeluTeensSite.BACKSTAGE || soundController.volume == 1) return;
			musicChannel = music.play( soundController.position );
			if (soundTween) soundTween.stop();

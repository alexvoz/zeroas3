package com.dmotiko.seluteens {
	...
	public class SeluTeensBackstageSection
	extends BaseClip {
		...
		override protected function initClip():void {
			...
			if ( SeluTeensSite.getApp().getSound() ) volumeController.volume = 1;
			else volumeController.volume = 0;
		}
		
		private function sound_changed(e:Event):void {
			if ( SeluTeensSite.getApp().getSection() != SeluTeensSite.BACKSTAGE) return;
		...
		private function video_change(e:VideoEvent):void {
			SeluTeensSite.getApp().log("video_change= " + e.state);
		}
		
		private function section_changed(e:Event):void {
			if ( SeluTeensSite.getApp().getSection() == SeluTeensSite.BACKSTAGE ) {
				flvPlayer.playWhenEnoughDownloaded();
				if ( !SeluTeensSite.getApp().getSound() ) return;
				videoFadeIn();
			} else {
				...
				SeluTeensSite.getApp().fadeInMusic();
			}
		}
		...
		private function videoFadeIn():void	{
			...
			tVolume.addEventListener( TweenEvent.MOTION_CHANGE, refresh_volume);
			SeluTeensSite.getApp().fadeOutMusic();
		}
